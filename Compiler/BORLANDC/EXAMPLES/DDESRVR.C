...
//
//*******************************************************************
HWND DDEServerInit(HWND hParentWnd, HWND hClientWnd, LONG lParam,
		  char *szApp, char *szTopic)
{
    ...
    SetWindowWord(hChannel, WW_CLIENT, hClientWnd);

    // Send acknowledgement to client.
    wLastDDEServerMsg = WM_DDE_ACK;
    SendMessage(hClientWnd, wLastDDEServerMsg, hChannel, MAKELONG(aApp, aTopic));
...
int DDEServerClose(HWND hChannel)
{
    ...
    if (IsWindow(hChannel))
    {
        ...
        hClientWnd = GetWindowWord(hChannel, WW_CLIENT);

        // If he is still active.
        if (IsWindow(hClientWnd))
            SendMessage(hClientWnd, wLastDDEServerMsg, hChannel, 0L);
    }
...
//
//*******************************************************************
LONG FAR PASCAL DDEServerWndProc(HWND hWnd, WORD message,
				 WORD wParam, LONG lParam)
{
    ...
    switch (message)
    {
	    ...
	    // Send acknowledgement back to client.
            PostMessage(hClientWnd, WM_DDE_ACK, hWnd,
                        MAKELONG(wStatus, hCommand));
            break;
	    ...
	    // Init return code.
		    ...
		    if (lpShData)
                    {
                        ...
                        if (lpData)
                        {
                            ...
                            _fstrcpy(lpShData->Value, lpData);
                            GlobalUnlock(hData);
            ...
            if (nRc)
            {
                ...
                PostMessage(hClientWnd, WM_DDE_DATA, hWnd,
                            MAKELONG(hShData, aItem));
            }
            else
            {
        ...
        case WM_DDE_TERMINATE:      // server says terminate conversation
            ...
            hClientWnd = GetWindowWord(hWnd, WW_CLIENT);

            // Ignore if not from our client.
            if (wParam != hClientWnd)
                return(0L);
		...
		PostMessage(hClientWnd, WM_DDE_TERMINATE, hWnd, 0L);
